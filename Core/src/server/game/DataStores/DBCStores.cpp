...
//Stitch INFO : MAX_RACES - entry->Race << 16 par defaut
              sCharSectionMap.insert({ entry->GenType | (entry->Gender << 8) | (entry->Race << 18), entry });
...
//Stitch INFO : MAX_RACES - entry->Race << 16 par defaut
	std::pair<CharSectionsMap::const_iterator, CharSectionsMap::const_iterator> eqr = sCharSectionMap.equal_range(uint32(genType) | uint32(gender << 8) | uint32(race << 18));
